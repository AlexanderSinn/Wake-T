...
class PlasmaParticleContainerPy:
    ...
    def __init__(self, serialized_list=None):
        if serialized_list is not None:
            self.r, self.log_r, self.dr_p, self.pr, self.pz, self.gamma, self.w, self.w_center, \
            self.r_to_x, self.id, self.mass, self.charge, \
                \
            self.psi, self.dr_psi, self.dxi_psi, self.b_t, self.b_t_0, self.nabla_a2, \
            self.a2, self.rho, self.chi, \
                \
            self.a_i, self.b_i, self.sum_1, self.sum_2, self.sum_3, self.a_0, self.A, self.B, \
            self.C, self.K, self.U, \
                \
            self.dr, self.dpr, \
                \
            self.r_hist, self.log_r_hist, self.xi_hist, self.pr_hist, self.pz_hist, self.w_hist, \
            self.r_to_x_hist, self.id_hist, self.sum_1_hist, self.sum_2_hist, self.a_i_hist, \
            self.b_i_hist, self.a_0_hist, self.i_push, self.xi_current, \
                \
            self.num_particles, self.is_ion, self.do_push, self.store_history \
                = serialized_list
